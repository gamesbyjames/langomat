...
// --- Ask user for ElevenLabs key at runtime (stored in localStorage) ---
let ELEVEN_LABS_API_KEY = localStorage.getItem('elevenLabsKey') || '';
if (!ELEVEN_LABS_API_KEY) {
  const entered = window.prompt('Please enter your full ElevenLabs API key (it will be kept locally on this computer):');
  if (entered && entered.trim().length > 0) {
    ELEVEN_LABS_API_KEY = entered.trim();
    localStorage.setItem('elevenLabsKey', ELEVEN_LABS_API_KEY);
  } else {
    // No key provided – fallback to Web Speech API only
